<div class="mb-3">
    ...
    <input type="text" class="form-control" id="id" name="id" value="{{ $movie->id ?? '' }}" {{ isset($movie) ? 'disabled' : 'required' }}>
</div>
<div class="mb-3">
...
<div class="mb-3">
    ...
    <select name="category_id" id="category_id" class="form-select" required>
        ...
        @foreach ($categories as $category)
            <option value="{{ $category->id }}" {{ (isset($movie) && $movie->category_id == $category->id) ? 'selected' : '' }}>{{ $category->nama_kategori }}</option>
        @endforeach
    </select>
</div>
<div class="mb-3">
...
@if(isset($movie) && $movie->foto_sampul)
<div class="mb-3">
